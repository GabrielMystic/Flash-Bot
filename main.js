...
global.authFile = `InfiniteBotSession`
...
auth: state,
browser: ['Infinite-Bot','Edge','1.0.0'],
version   
}
...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "InfiniteJadiBot"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '3', '-type', 'f', '-delete'])))
}, 30 * 1000)}
...
function purgeSession() {
    let prekey = []
    let directorio = readdirSync("./InfiniteBotSession")
    let filesFolderPreKeys = directorio.filter(file => {
        return file.startsWith('pre-key-')
    })
    prekey = [...prekey, ...filesFolderPreKeys]
    filesFolderPreKeys.forEach(files => {
    unlinkSync(`./InfiniteBotSession/${files}`)
})

}  
function purgeSessionSB() {
let listaDirectorios = readdirSync('./InfiniteJadiBot/');
console.log(listaDirectorios)
      let SBprekey = []
listaDirectorios.forEach(filesInDir => {
    let directorio = readdirSync(`./InfiniteJadiBot/${filesInDir}`)
    ...
    DSBPreKeys.forEach(fileInDir => {
        unlinkSync(`./InfiniteJadiBot/${filesInDir}/${fileInDir}`) 
    })
    })
...
const directories = ['./InfiniteBotSession/', './InfiniteJadiBot/']
